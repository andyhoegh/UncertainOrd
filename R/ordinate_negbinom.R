...
ordinate_negbinom <- function(num.mcmc, Y, burn.in = 100){
  # Initialize Storage
  mcmc <- boral::boral(Y, family = 'negative.binomial', lv.control = list(num.lv = 2), row.eff = 'fixed', save.model =T, prior.control = list(type=c('normal','normal','normal','halfcauchy'), hypparams = c(1,1,1,20) ))
