...
BinaryHPD <- function(n, p, mu.alpha = 0, sd.alpha = 1, mu.beta = 0, sd.beta = 1){
  ...
  pres.mcmc <- boral::boral(sim.vals$mat, family = 'binomial', num.lv = 2, row.eff = 'fixed', save.model =T, prior.control = list(type=c('normal','normal','normal','halfcauchy'), hypparams = c(1,1,1,20) ))
